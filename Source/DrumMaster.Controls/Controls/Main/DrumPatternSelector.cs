...
namespace Restless.App.DrumMaster.Controls
{
    ...
    public class DrumPatternSelector : PatternSelector
    {
        #region Private
        ...
        internal DrumPatternSelector(SongContainer owner, SongDrumPatternSelectorType type) : base(type)
        {
            Owner = owner ?? throw new ArgumentNullException(nameof(owner));
            DrumPattern = new DrumPattern(null);
            DrumPattern.DisplayName = "what name";
        ...
        {
            foreach (var point in hostGrid.Children.OfType<SongPointSelector>())
            {
        ...
        private void CreateHostGrid()
        {
            if (hostGrid != null)
            {
                ...
                for (int k = 1; k <= Constants.SongSelector.Count.Default; k++)
                {
                    ...
                    SongPointSelector point = new SongPointSelector(this, SelectorType)
                    {
                        Margin = new Thickness(1),
                        Position = k,
                        //DisplayName = SelectorType == SongDrumPatternSelectorType.Header ? $"{k}" : null,
                    };
        ...
        private void ChangeDivisionSeparators()
        {
            ...
            while (insertIdx < hostGrid.Children.Count - 1)
            {
                ...
                foreach (var child in hostGrid.Children.OfType<SongPointSelector>())
                {
        ...
        {
            Owner.Owner.DrumKitContainer.ActivateDrumPattern(DrumPattern); 
        }
        #endregion
    }
}
